 ...
 * @author win7
 */
class MessageAddAction extends BaseAppAction with SkipCsrfCheck {
  def execute(): Unit = {
    log.info(s"Start MessageAddAction")
    val messageBean = getBean[TenantMessage](paramo("message"))
    ActorHolder.proxyActor ! TEvent(HfbkUtil.getUUID(), TenantMessageAddBiz, messageBean, HfbkUtil.getTime(), Some(self))
