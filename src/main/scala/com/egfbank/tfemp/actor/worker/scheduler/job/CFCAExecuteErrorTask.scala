 ...
 * @author andy
 */
class CFCAExecuteErrorTask extends SchedulerWorker with CFCADbService {

  //定时扫描下行指令表-找出因"网络连接失败"而导致的任务失败
  def start() = {
    log.info("Start CFCAExecuteErrorTask")
    val tasks: List[Map[String, String]] = selectDownQueueError4Exe()
