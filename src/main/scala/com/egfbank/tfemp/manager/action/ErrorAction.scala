 ...
 */
@POST("/error/errorXml")
class ErrorAction extends AppAction with SkipCsrfCheck with CFCADbService with Log {
  def execute = {
    requestMapString match {
      case paramMap: Map[String, String] => {
        log.info(s"receive paramMap,${paramMap}")
        val list = selectError(paramMap.filter( x => !x._2.trim().equals("")))
      ...
      case _ => {
        ...
        //        respondJson(AppQueryResult("1", "get requestParam failed", 0, null))
      }
    }
  }
}